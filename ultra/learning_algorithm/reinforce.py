...
class Reinforce(BaseAlgorithm):
    ...
    def train(self, input_feed):
        ...
        ranking_prob = torch.gather(train_probs, dim=1, index=ranking)
        self.loss = torch.tensor(0, dtype=torch.float32, device=self.device,
                                 requires_grad=True)
        with torch.no_grad():
            reward = torch.zeros_like(train_probs, dtype=torch.float32, device=self.device)
            for k in range(reward.shape[1]):
                reward[:, k] = self.dcg(labels_sorted_via_preds[:, :k + 1]) / self.dcg(ideally_sorted_labels[:, :k + 1])
            # reward = dcg(action_rel, probs.shape[0]) / idcg(action_rel, probs.shape[0])
            # alpha(d | q) in ranking order
        topk_grad = train_probs.shape[1] * (1 - ranking_prob) ** (train_probs.shape[1] - 1)

        # self.loss = self.softmax_loss(train_output, train_labels)
        self.loss = torch.sum(reward * topk_grad)
