...
class BaseAlgorithm(ABC):
        ...
        for i in range(len(output_scores)):
            if self.is_cuda_avail:
                output_scores[i] = torch.where(
                    input_flag_list[i].to(device=self.device),
                    output_scores[i],
                    torch.ones_like(output_scores[i],device=self.device) * self.PADDING_SCORE
                )
            else:
                output_scores[i] = torch.where(
                    input_flag_list[i],
    ...
    def create_input_feed(self, input_feed, list_size):
        ...
        if self.is_cuda_avail:
            self.labels = self.labels.to(device=self.device)
        self.docid_inputs = torch.as_tensor(data=np.array(self.docid_inputs, dtype=np.int64))
